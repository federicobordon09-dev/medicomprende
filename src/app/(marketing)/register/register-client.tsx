...
function GoogleSignUpCard() {
  return (
    <motion.div
      className="bg-white rounded-2xl border border-azul-200/50 p-6 sm:p-8 shadow-lg min-w-0 w-full"
      initial={{ opacity: 0, x: 24 }}
      animate={{ opacity: 1, x: 0 }}
      transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
    >
      <div className="mx-auto w-12 h-12 rounded-xl bg-azul-100 flex items-center justify-center mb-5">
        <UserPlusIcon />
      </div>
      <h2 className="font-display font-bold text-xl text-warm-950 mb-2 text-center">
        Creá tu cuenta gratis
      </h2>
      <p className="text-sm text-warm-600 mb-6 leading-relaxed text-center">
        Accedé a todas las herramientas de <strong>MediComprende</strong> con tu cuenta de Google.
      </p>
      <button
        onClick={() => signIn("google", { callbackUrl: "/dashboard" })}
        className="w-full flex items-center justify-center gap-3 bg-white border border-azul-300/50 hover:bg-azul-50/80 text-warm-900 font-semibold px-4 py-3 rounded-xl shadow-sm hover:shadow-md active:scale-[0.98] transition-all duration-150"
      >
        ...
        <span className="truncate">Registrarme con Google</span>
      </button>
      <p className="text-xs text-warm-400 text-center mt-4 leading-relaxed">
        Al registrarte aceptás nuestros{" "}
        <Link href="/terminos" className="text-xs text-warm-400 hover:text-warm-600 hover:underline">términos de servicio</Link>{" "}
        y{" "}
        <Link href="/privacidad" className="text-xs text-warm-400 hover:text-warm-600 hover:underline">política de privacidad</Link>.
      </p>
    </motion.div>
  );
}
    ...
    <section className="relative min-h-[80dvh] flex items-center py-16 sm:py-20 px-4 sm:px-6 overflow-hidden bg-gradient-to-b from-azul-50 via-white to-azul-50/40">
      ...
      <div className="relative z-10 w-full max-w-5xl mx-auto">
        <div className="flex flex-col lg:grid lg:grid-cols-5 gap-6 lg:gap-12 items-center">
          ...
          <div className="order-1 lg:order-2 lg:col-span-2 lg:pl-6 min-w-0">
            <GoogleSignUpCard />
          </div>
        </div>
      </div>
