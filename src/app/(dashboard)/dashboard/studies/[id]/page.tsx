...
export default function StudyDetailPage() {
  ...
  if (error || !study) {
    return (
      <div className="max-w-lg mx-auto text-center py-16">
        ...
        <button onClick={() => router.push("/dashboard")} className="text-coral-500 font-medium">Volver al dashboard</button>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      {analysis ? (
        <>
          <div className="flex gap-1 bg-sk-100 rounded-xl p-1 overflow-x-auto">
            {tabs.map((tab) => (
              <button
                key={tab.id}
              ...
              >
                ...
                {tab.count && tab.count > 0 ? (
                  <span className="w-5 h-5 rounded-full bg-coral-500 text-white text-[10px] flex items-center justify-center">
                    {tab.count}
                  </span>
                ) : null}
              </button>
          ...
          <div className="space-y-6 animate-[fadeInUp_0.3s_ease-out]">
            {activeTab === "summary" && (
              <>
                <div className="bg-gradient-to-br from-sk-800 to-sk-950 rounded-2xl p-6 md:p-8 shadow-lg">
                  ...
                  <p className="text-sk-100 leading-relaxed">{analysis.summary}</p>
                </div>
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {analysis.outOfRangeValues.map((v, i) => {
                          const statusColor = v.status === "normal" ? "bg-mint-100 text-mint-700"
              ...
              <div className="space-y-4">
                ...
                {analysis.parameterExplanations.length === 0 ? (
                  ...
                  analysis.parameterExplanations.map((p, i) => (
                    <div key={i} className="bg-white rounded-2xl p-6 shadow-md border border-sk-100">
                      <div className="flex items-center gap-3 mb-3">
                        <div className="w-10 h-10 rounded-xl bg-sk-100 flex items-center justify-center font-bold text-sk-600">
                          {i + 1}
                        </div>
                        <div>
                      ...
                      {p.possibleCauses.length > 0 && (
                        <div className="mt-3 pt-3 border-t border-sk-100">
            ...
            {activeTab === "questions" && (
              <div className="bg-white rounded-2xl p-6 md:p-8 shadow-md border border-coral-100">
                ...
                {analysis.suggestedQuestions.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {analysis.suggestedQuestions.map((q, i) => (
                      <div key={i} className="flex items-start gap-3 bg-sk-50 rounded-xl p-4 group">
                        <span className="text-coral-500 mt-0.5">❓</span>
                        <p className="flex-1 text-warm-900">{q}</p>
                        <button
                          onClick={() => copyText(q, `q-${i}`)}
                          className="flex-shrink-0 text-xs text-coral-500 hover:text-coral-600 font-medium opacity-0 group-hover:opacity-100 transition-opacity"
                        >
                    ...
                    <button
                      ...
                      className="w-full mt-2 text-sm text-coral-500 hover:text-coral-600 font-medium py-2"
                    >
                ...
                {analysis.medicalTerms.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {analysis.medicalTerms.map((t, i) => (
                      <div key={i} className="border border-sk-200 rounded-xl p-4 hover:shadow-sm transition-shadow">
        ...
        <div className="bg-white rounded-2xl p-8 text-center shadow-md">
          ...
          <button
            ...
            className="bg-coral-500 hover:bg-coral-600 text-white font-semibold px-6 py-2.5 rounded-xl transition-all"
          >
            Analizar ahora
          </button>
        </div>
