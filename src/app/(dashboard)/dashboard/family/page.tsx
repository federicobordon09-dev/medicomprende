...
export default function FamilyPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        ...
        {profiles.length < 5 && (
          <button
            ...
            className="bg-coral-500 hover:bg-coral-600 text-white text-sm font-semibold px-4 py-2 rounded-xl transition-all"
          >
      ...
      {showForm && (
        <form onSubmit={createProfile} className="bg-white rounded-xl p-6 border border-sk-200/60 space-y-4">
          <h3 className="font-display font-semibold text-lg">Nuevo perfil</h3>
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-2.5 border border-sk-200 rounded-xl text-warm-900 focus:outline-none focus:ring-2 focus:ring-coral-500"
              required
            />
          </div>
          <div>
            ...
            <div className="flex gap-2">
              {COLORS.map((c) => (
                <button
                  ...
                  type="button"
                  onClick={() => setColor(c)}
                  className={`w-8 h-8 rounded-full transition-all ${color === c ? "ring-2 ring-offset-2 ring-warm-950 scale-110" : ""}`}
          ...
          <button
            type="submit"
            className="w-full bg-coral-500 hover:bg-coral-600 text-white font-semibold py-2.5 rounded-xl transition-all"
          >
            Crear perfil
          </button>
        </form>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {profiles.map((p) => (
          <div key={p.id} className="bg-white rounded-xl p-5 border border-sk-200/60 card-hover">
            <div className="flex items-center justify-between">
